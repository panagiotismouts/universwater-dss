"""
WINGS water sensor API client.

Fetches all water quality variables from the WINGS SensorThings (OGC) API
for all water sensors (Aquaread ×2, HCMR) defined in config.yaml wings.sensors.

Authentication: OAuth2 client_credentials via WingsTokenManager.

Observations endpoint:
  GET {base_url}/api/v1/collections/sensor_things:observations
  Params:
    version=v2
    fields=phenomenon_time_start,result_number
    limit={page_size}
    offset={offset}
    filter=JSON filter object (datastream_id + time range)

Pagination: limit/offset; stops when response list is shorter than page_size.
...
def _load_wings_water_datastreams() -> dict[str, dict[str, str]]:
    """
    Return {sensor_id: {variable_name: datastream_id}} for all water sensors.

    Water sensors are those that have at least one of the canonical water
    variables (ph, dissolved_oxygen, temperature_water, turbidity, conductivity).
    """
    _water_variables = {
        "ph", "dissolved_oxygen", "temperature_water", "turbidity", "conductivity",
        "orp", "do_saturation", "tds", "salinity", "ammonia", "nitrate",
    ...
    for s in sensors:
        ...
        if any(v in streams for v in _water_variables):
            result[s["sensor_id"]] = streams
    return result


class WingsWaterClient(BaseAPIClient):
    """WINGS SensorThings client for water quality variables."""
    ...
    async def fetch(
        ...
        sensor_map = _load_wings_water_datastreams()
        ...
        for sensor_id, datastreams in sensor_map.items():
            ...
            if datastream_id is None:
                continue  # this sensor doesn't carry this variable
    ...
    async def _get_with_retry(
        ...
        for attempt in range(1, settings.wings_max_retries + 1):
            try:
                ...
                return response.json()  # list of observation dicts

            except httpx.TimeoutException as exc:
                last_exc = exc
                log.warning("wings_water_timeout", variable=variable_name, attempt=attempt)
                if attempt < settings.wings_max_retries:
                    await asyncio.sleep(_RETRY_BACKOFF_SECONDS)
                ...
                last_exc = exc
                log.warning("wings_water_fetch_error", variable=variable_name, attempt=attempt, error=str(exc))
                if attempt < settings.wings_max_retries:
                    await asyncio.sleep(_RETRY_BACKOFF_SECONDS)

        if isinstance(last_exc, httpx.TimeoutException):
            raise SourceAPITimeoutError(
                f"WINGS water timeout after {settings.wings_max_retries} attempts for {variable_name}"
            ) from last_exc
        raise SourceAPIError(
            f"WINGS water fetch failed after {settings.wings_max_retries} attempts for {variable_name}"
        ) from last_exc
